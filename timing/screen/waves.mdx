...
<Frame caption="Demo data.">
  <img src="/images/timing/wave-controls.png" alt="Three demo waves with two live clocks and one waiting to start" />
</Frame>
...
When the required start and finish records are available, the timer calculates:

| | Measured from | Used for |
|---|---|---|
| **Net time** | Their wave’s start, or the configured fallback | Elapsed time from that rider’s start group |
| **Gun time** | The race start | Elapsed time from the race gun |

Check which measure your event uses for ranking and awards. A displayed net
or gun time does not determine the scoring rule on its own.

<Tip>
  A recorded wave start takes precedence over its planned offset. A wave's planned offset is only a fallback
  for working out net time if that wave never got started live. So if you set
  "+3 minutes" and recorded release at 3:40, the 3:40 is used. A planned offset
  does not confirm when riders actually left; correct a missed start from your
  race records.
</Tip>
