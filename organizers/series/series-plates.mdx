---
title: "Series plates"
description: "Assign season plate numbers for races in a series."
icon: "hashtag"
---
...
Manage plates from the **Plates** tab of the series.

<Frame caption="Example screen with fictional demo data.">
  <img src="/images/organizers/series-plates.png" alt="Series plate table with rider numbers, assignment status, and filters" />
</Frame>

## Assign a plate

1. Open the **…** menu and click **Assign plate**.
...
Click **Import CSV** to upload a `plate,participantId` CSV. Review any preview
and confirmation offered before importing, then check the row-level results:
...
The public series page carries a **Season plates** section. Anyone gets the
policy — whether numbers are kept for the season, and what a replacement costs —
and a signed-in rider also sees their own number, when they were first given it,
and how many rounds they've used it at. They see only their own and those of
riders they manage.

That section only appears once a plate policy is set on the series. Without one,
a rider holding number 143 all season has no way to look it up and emails you
instead.

→ [Series pages](/riders/finding-events/org-and-series-pages#season-plates)
